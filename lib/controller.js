...
class Controller {
        ...
        this.startupLogVersion(() => {
            this.zigbee.start(this.handleZigbeeMessage, (error) => {
                if (error) {
					...
					// TODO: CONF DOMOTICZ
					if (settings.get().domoticz) {
                        subscriptions.push('domoticz/in');
                    }
    ...
    handleMQTTConnected() {
		...
		if (settings.get().domoticz) {
            ...
            this.zigbee.getAllClients().forEach((device) => {
                const mappedModel = zigbeeShepherdConverters.findByZigbeeModel(device.modelId);
                if (mappedModel) {
                    domoticz.discover(device.ieeeAddr, mappedModel.model, this.mqtt, true);
                }
            });
        }
    ...
    handleZigbeeMessage(message) {
		...
		// TODO: CONF DOMOTICZ
		// Domoticz MQTT discovery
        if (settings.get().domoticz) {
            domoticz.discover(device.ieeeAddr, mappedModel.model, this.mqtt, false);
        }
    ...
    handleMQTTMessageConfig(topic, message) {
        ...
        if (option === 'permit_join') {
            ...
            const cleanup = () => {
				...
				// TODO: CONF DOMOTICZ
				// Clear Domoticz MQTT discovery message
                if (settings.get().domoticz && device) {
                    const mappedModel = zigbeeShepherdConverters.findByZigbeeModel(device.modelId);
                    if (mappedModel) {
                        domoticz.clear(deviceID, mappedModel.model, this.mqtt);
                    }
                }
