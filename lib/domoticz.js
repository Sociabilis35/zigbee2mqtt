...
// Map domoticz configurations to devices.
const mapping = {
...
function discover(deviceID, model, mqtt, force) {
    ...
    mapping[model].forEach((config) => {
        ...
        mqtt.publish(topic, JSON.stringify(payload), {retain: true, qos: 0}, null, 'domoticz');
    });
...
function clear(deviceID, model, mqtt) {
    ...
    mapping[model].forEach((config) => {
        ...
        const topic = 'in'
        const payload = '';
        mqtt.publish(topic, payload, {retain: true, qos: 0}, null, 'domoticz');
    });
